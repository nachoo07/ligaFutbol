...
const UsersProvider = ({ children }) => {
    ...
    const addUsuarioAdmin = async (usuario) => {
        if (auth === 'admin') {
            try {
                const response = await axios.post("http://localhost:4002/api/users/create", usuario, {
                    withCredentials: true,
                });
    ...
    const updateUsuarioAdmin = async (id, usuarioActualizado) => {
        if (auth === 'admin') {
            try {
                const response = await axios.put(`http://localhost:4002/api/users/update/${id}`, usuarioActualizado, {
                    withCredentials: true,
                });
        ...
        if (auth === 'admin') {
            try {
                ...
                if (confirmacion.isConfirmed) {
                    await axios.delete(`http://localhost:4002/api/users/delete/${id}`, {
                        withCredentials: true,
                    });
